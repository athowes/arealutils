...
#' When the average distance between points is below `1e-8` returns value one.
#' 
...
best_average <- function(D, kernel = matern, p = 0.01, ...) {
  ...
  # When points are all the same (such that there is no distance between them) return l = 1
  if(m < 1e-8) return(1)
